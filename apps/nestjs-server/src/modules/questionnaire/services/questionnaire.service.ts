...
import {
	...
	StreamAverageDto,
} from "../dto/response/questionnaire-response.dto";
...
export class QuestionnaireService {
	...
	private async getRiskItems(): Promise<DictionaryItemDto[]> {
		...
		try {
			const _options = JSON.parse(JSON.stringify(riskItem.options));
			...
			return riskItem.options.map((option: any) => {
				...
				const coeff = riskCoefficients.find(
					(c) => c.code === `risk_${cleanValue}`,
				);
	...
	async getFullQuestionnaire(): Promise<QuestionnaireResponseDto> {
		const [items, coefficients, streamAverages, referenceData] =
			await Promise.all([
		...
		for (const avg of streamAverages) {
			streamAveragesDto[avg.epicName as keyof StreamAverageDto] =
				avg.averageValue;
		}
	...
	async getQuestionnaireItemByCode(
		code: string,
	): Promise<QuestionnaireItemEntity | null> {
