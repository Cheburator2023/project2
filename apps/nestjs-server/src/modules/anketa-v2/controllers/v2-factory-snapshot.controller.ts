...
export class V2FactorySnapshotController {
	constructor(
		...
		private readonly factoryTypicalWorksPublishService: V2FactoryTypicalWorksPublishService,
	) {}
	...
	@Post("publish-typical-works")
	@DomainRoles("appadmin", "sacfg")
	@ApiOperation({
		summary:
			"Publish типовых работ версии в factory registry+catalog (dry-run по умолчанию; write при V2_FACTORY_PUBLISH_WRITE=1)",
	})
	async publishTypicalWorks(
		@Body() body: V2FactoryPublishTypicalWorksRequestDto,
	): Promise<V2FactoryPublishTypicalWorksResponseDto> {
		return this.factoryTypicalWorksPublishService.publish(body);
	}
}
