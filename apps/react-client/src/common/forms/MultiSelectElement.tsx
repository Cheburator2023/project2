...
import React, { ReactNode, Ref, RefAttributes, forwardRef } from "react";
import {
	Control,
	FieldError,
	FieldPath,
...
>(
	...
	type OptionType = Record<string, any> | string | number;
	type MultiSelectElementProps<
	...
	// Update the renderLabel function to properly type the parameters
	const renderLabel = (item: any) => {
		const foundOption: any = options.find((op: any) => {
			if (typeof op === "object" && op !== null) {
				const optionVal = itemValue || itemKey ? op[itemValue || itemKey] : op;
				return optionVal === item;
			}
			return op === item;
		});

		if (foundOption && typeof foundOption === "object" && itemLabel) {
			return foundOption[itemLabel] || item;
		}
