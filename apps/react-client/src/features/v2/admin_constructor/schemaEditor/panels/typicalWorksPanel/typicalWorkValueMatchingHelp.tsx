...
> = {
	...
	labor: {
		...
		lines: [
			"«По значениям» — отдельный коэффициент на каждый вариант (справочник, enum, чекбокс Да/Нет).",
			"Any-of — coeffOn, если значение попало в отмеченное множество, иначе coeffOff. Сопоставление такое же, как в триггерах.",
			"Параметр привязывается к полю схемы (@ field_…). Без токена в формуле (коэф / anyof) итоговый коэффициент работы останется 1.",
		],
	},
	formula: {
