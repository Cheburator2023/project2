...
export type RegistryRoutes = {
	calculationCreate: { rootPath: string };
	calculationPreview: { rootPath: string };
	calculationNewVersion: { rootPath: string };
	calculationClone: { rootPath: string };
	calculationCompare: { rootPath: string };
};
...
function buildRegistryUrl(relativePath: string, pathPrefix: string): string {
	const route = relativePath.replace(/^\//, "");
	const prefix = pathPrefix.replace(/\/$/, "");
	const app = APP_NAME ? `/${APP_NAME}` : "";
	return `${location.origin}${app}${prefix ? `${prefix}/` : "/"}${route}`.replace(
		/([^:]\/)\/+/g,
		"$1",
	);
}
...
export const HomeTemplete = ({
	...
	const onCreateCalculation = () => {
		navigate(registryRoutes.calculationCreate.rootPath);
	};
	...
	const onRowDoubleClicked = (event: RowDoubleClickedEvent) => {
		...
		if (calculationId) {
			navigate(
				registryRoutes.calculationPreview.rootPath.replace(
					":id",
	...
	const getContextMenuItems = (params: GetContextMenuItemsParams) => {
		...
		const customItems = [
			...
			},
			{
				...
				action: () => {
					if (calculationId) {
						const route = registryRoutes.calculationNewVersion.rootPath.replace(
							":id",
							calculationId.toString(),
						);
						window.open(buildRegistryUrl(route, pathPrefix), "_blank");
			...
			},
			{
				...
				action: () => {
					if (calculationId) {
						const route = registryRoutes.calculationClone.rootPath.replace(
							":id",
							calculationId.toString(),
						);
