...
import {
	...
	mergeV2QuestionnaireRegistryColumnTrees,
...
describe("v2-questionnaire-registry-columns.util", () => {
	it("uses dotted form column ids", () => {
		expect(registryFormColumnId("detailInfo.parameters.streamNames")).toBe(
			"form.detailInfo.parameters.streamNames",
		);
	});
	...
	it("localizes risk group headers from schema titles", () => {
		...
		const riskCol = columns.find((col) =>
			col.key.includes("riskGroup.businessComplexity"),
		);
	...
	it("includes schema parameters and model list fields", () => {
		const leaves = flattenV2RegistryColumnTree(
			buildV2QuestionnaireRegistryColumnTree(MINIMAL_SCHEMA, MINIMAL_UI, {
				arrayMaxItems: 1,
			}),
		);
	...
	it("buildV2QuestionnaireRegistryConfig merges schemas", () => {
		...
		const ids = flattenV2RegistryColumnTree(config.columnTree).map(
			(leaf) => leaf.id,
		);
		expect(ids).toContain("form.generalInfo.fieldA");
		expect(ids).toContain("form.generalInfo.fieldB");
