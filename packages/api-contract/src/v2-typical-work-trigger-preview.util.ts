...
import {
	hasTypicalWorkPreviewFormContext,
...
export function canEvaluateTypicalWorkTriggerPreview(
	draftSource: Record<string, unknown> | undefined,
	previewFormData: Record<string, unknown> | undefined,
	matchContext?: TypicalWorkTriggerMatchContext,
): boolean {
	if (
		draftSource &&
	...
	return hasTypicalWorkPreviewFormContext(
		previewFormData,
		matchContext?.uiSchema,
	);
}

function buildTriggerMatchInput(
	input: TypicalWorkTriggerPreviewInput,
): Parameters<typeof hasTypicalWorkTriggersConfigured>[0] {
	const normalized = normalizeTypicalWorkTriggerRulesForMatch(input.rules);
	const rules = normalized.map((rule) =>
		input.mapRule ? input.mapRule(rule) : rule,
	);
	return {
 ...
 */
export function evaluateTypicalWorkTriggerPreview(
	input: TypicalWorkTriggerPreviewInput,
): TypicalWorkTriggerPreviewResult {
	...
	if (
		!canEvaluateTypicalWorkTriggerPreview(
			input.draftSource,
			input.previewFormData,
			input.matchContext,
		)
	) {
		return {
