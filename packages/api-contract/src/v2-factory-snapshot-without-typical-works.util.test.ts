...
import {
	...
	stripTypicalWorksLogicRules,
	type V2TemplateSnapshotLike,
} from "./v2-factory-snapshot-without-typical-works.util";
...
function loadDefaultSnapshot(): V2TemplateSnapshotLike {
	return JSON.parse(readFileSync(defaultSnapshotPath, "utf8")) as V2TemplateSnapshotLike;
}
...
describe("prepareFactorySnapshotWithoutTypicalWorks", () => {
	it("disables catalog bindings and strips typical-works logic from factory snapshot", () => {
		...
		expect(
			strippedIds.some((id) => id.startsWith("typical-works-catalog-")),
		).toBe(false);
	});
	...
	it("stripTypicalWorksLogicRules leaves non-typical rules intact", () => {
		const snapshot = loadDefaultSnapshot();
		const stripped = stripTypicalWorksLogicRules(
			snapshot.logic as V2LogicGraphDto,
			snapshot.uiSchema,
		);
