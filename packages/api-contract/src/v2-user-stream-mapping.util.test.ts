...
describe("v2-user-stream-mapping", () => {
	it("maps _rnd / rnd to RnD + AI-модели партнерств", () => {
		expect(
			resolveV2UserScopedStreamsFromGroups(["sum_ds_rnd"]),
		).toEqual(
		...
		expect(resolveV2UserScopedStreamsFromGroups(["rnd"])).toEqual(
			expect.arrayContaining([
				V2_IMPLEMENTATION_STREAM.RND,
				V2_IMPLEMENTATION_STREAM.PTITPC,
			]),
		);
		const allowed = resolveV2UserAllowedStreamFilterValues(
			["ds", "sum_ds_rnd"],
	...
	it("de/modelops view all streams by default", () => {
		...
		expect(
			isV2UserStreamFilteredByGroups(["modelops", "sum_mo_rnd"]),
		).toBe(false);
	...
	it("de/modelops are filtered when deModelopsViewAllStreams=false", () => {
		expect(
			isV2UserStreamFilteredByGroups(["de", "sum_de_rb"], {
				deModelopsViewAllStreams: false,
			}),
		).toBe(true);
		expect(
			isV2UserStreamFilteredByGroups(["modelops", "sum_mo_rb"], {
				deModelopsViewAllStreams: false,
			}),
		).toBe(true);
		/** leads remain exempt */
		expect(
			isV2UserStreamFilteredByGroups(["de_lead"], {
				deModelopsViewAllStreams: false,
			}),
		).toBe(false);
	});
	...
	it("stream_view_all bypasses filter for any role", () => {
		expect(
			isV2UserStreamFilteredByGroups(
				["ds", "sum_ds_rb", V2_USER_STREAM_VIEW_ALL_ROLE_CODE],
				{ deModelopsViewAllStreams: false },
			),
		).toBe(false);
		expect(
			isV2UserStreamFilteredByGroups(["/stream_view_all", "ds"], {
				deModelopsViewAllStreams: false,
			}),
		).toBe(false);
		expect(
			isV2UserStreamFilteredByGroups(["sum_stream_view_all", "ds"], {
				deModelopsViewAllStreams: false,
			}),
		).toBe(false);
	});
	...
	it("filter keeps rnd+ptitpc questionnaires for ds_rnd", () => {
		...
		const filtered = filterV2QuestionnairesByUserStreamGroups(
			items,
			["ds", "sum_ds_rnd"],
			true,
			{ deModelopsViewAllStreams: false },
