...
describe("v2-factory-typical-works-publish.util", () => {
	it("extracts stage prefixes", () => {
		...
		expect(
			extractPublishWorkStage("1. Качество модельных данных [КД]"),
		).toBeNull();
	});

	it("builds registry and upserts catalog with formula and any_of", () => {
		const existing: V2FactoryCatalogWorkRow[] = [
			{
				stream: "ПиРМ",
				component: "Модель",
				stage: "",
		...
		expect(result.report.dropped.some((d) => d.field.includes("triggerMode"))).toBe(
			true,
		);

		const updated = result.catalogRows.find(
			(row) =>
				buildFactoryCatalogRowKey(row) ===
				buildFactoryCatalogRowKey({
					component: "Система-источник",
					stage: "02",
					name: "Поиск данных",
	...
	it("merges legacy ИД streams into Источники данных without duplicates", () => {
		...
		const updated = result.catalogRows.find((row) =>
			row.name.includes("процесса загрузки"),
		);
		...
		expect(result.catalogRows.some((row) => row.name === "ПиРМ only")).toBe(
			true,
		);
	});
});
