...
import {
	buildFormulaFromCsvRow,
	buildFormulaTextFromCsvCore,
	csvRowToCatalogPatch,
	extractFormulaCoreFromCsvText,
...
2. Param B";"ЧД(работы) = ОКРУГЛ.ВВЕРХ(Норматив × [Param A] × [Param B]; 0,1)"`;
	...
	it("keeps commas inside trigger values and splits multiline triggers", () => {
		expect(
			parseCsvTriggerRules(
				"Тип системы-источника (внешний), Пилот (первичный, повторный)\nНеобходима продуктивизация",
			),
		).toEqual([
			{
...
ИД. Внешний;Арх. Компонент. Объект данных;Work;Этап 220. Work;Этап 220;Опциональная;0,8;Trigger;"— (параметры те же, что у этапа 217)";"ЧД(работы) = ОКРУГЛ.ВВЕРХ(Норматив; 0,1)"`;
	...
	it("extracts CEIL formula core", () => {
		const extracted = extractFormulaCoreFromCsvText(
			"ЧД(работы) = ОКРУГЛ.ВВЕРХ(Норматив × [Сложность предметной области] × [NDA]; 0,1)",
		);
		expect(extracted?.core).toBe(
			"Норматив × [Сложность предметной области] × [NDA]",
		);
	...
	it("maps Excel digit count ;2 to step 0.01", () => {
		const extracted = extractFormulaCoreFromCsvText(
			"ЧД(работы) =ОКРУГЛ.ВВЕРХ(Норматив × [Кол-во моделей]; 2)",
		);
		expect(extracted?.roundingMode).toBe("CEIL");
	...
	it("builds full formula from csv row", () => {
		const build = buildFormulaFromCsvRow(
			{
				formulaRaw:
					"ЧД(работы) = ОКРУГЛ.ВВЕРХ(Норматив × [Сложность предметной области]; 0,1)",
	...
	it("parses per-work coefficient values from formula prose", () => {
		const parsed =
			parseCsvLaborCoefficients(`Переменные — параметры трудоёмкости (Ki, по умолчанию = 1):
...
Исходная (калибровочная) формула: ЧД(работы) = ОКРУГЛ.ВВЕРХ(0,5 × (1 + 0,46 × ([Пилот] × [Сложность предметной области])); 0,1)`,
	...
	it("rejects an override that disconnects formula and labor param codes", () => {
		const row = parseCsvFormulaImportRows(
			`Стрим;Арх. Компонент;Название оригинальное;Название в смарт-анкете СУМ;Контекст;Тип работы;Наличие норматива;Параметр-триггер;Параметры трудоемкости;Формула
ИД. Внутренний;Арх. Компонент. Система-источник;Work;Этап 210. Work;Этап 210;Опциональная;1;Тип системы-источника (внутренний);1. Наличие реплики в DAPP;"ЧД(работы) = ОКРУГЛ.ВВЕРХ(Норматив × [Наличие реплики в DAPP]; 0,1)"`,
		)[0]!;
