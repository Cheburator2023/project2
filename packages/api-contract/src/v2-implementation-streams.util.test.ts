...
import {
	buildImplementationStreamEnumPair,
...
describe("v2-implementation-streams.util", () => {
	it("exposes stable codes with stream labels (keys ≤ 6 chars)", () => {
		...
		).toBe("Разработка моделей КМБ и КСБ");
		expect(V2_IMPLEMENTATION_STREAM_LABELS[V2_IMPLEMENTATION_STREAM.RB]).toBe(
			"Моделирование РБ",
		);
		expect(
			V2_IMPLEMENTATION_STREAM_LABELS[V2_IMPLEMENTATION_STREAM.PTITPC],
		).toBe("AI-модели партнерств");
		expect(
			V2_IMPLEMENTATION_STREAM_LABELS[V2_IMPLEMENTATION_STREAM.FINMDL],
		).toBe("Финансовое моделирование");
		expect(V2_IMPLEMENTATION_STREAM_LABELS[V2_IMPLEMENTATION_STREAM.RND]).toBe(
			"Моделирование RnD",
		);
		expect(
		...
		expect(V2_IMPLEMENTATION_STREAM_LABELS[V2_IMPLEMENTATION_STREAM.DADM]).toBe(
			"ДАДМ",
		);
		expect(V2_IMPLEMENTATION_STREAM_LABELS[V2_IMPLEMENTATION_STREAM.PIRM]).toBe(
			"Платформы и Решения для моделирования",
		);
		expect(
			V2_IMPLEMENTATION_STREAM_LABELS[V2_IMPLEMENTATION_STREAM.STRDAT],
		).toBe("Потоковые данные");
		expect(
			V2_IMPLEMENTATION_STREAM_LABELS[V2_IMPLEMENTATION_STREAM.DIGAGT],
		).toBe("Цифровые агенты");
	});
	...
	it("validates codes and resolves labels", () => {
		expect(isV2ImplementationStreamCode(V2_IMPLEMENTATION_STREAM.RB)).toBe(
			true,
		);
		expect(isV2ImplementationStreamCode(V2_IMPLEMENTATION_STREAM.PIRM)).toBe(
			true,
		);
		expect(isV2ImplementationStreamCode("Моделирование РБ")).toBe(false);
		expect(resolveImplementationStreamLabel(V2_IMPLEMENTATION_STREAM.RB)).toBe(
			"Моделирование РБ",
		);
		expect(
			resolveImplementationStreamLabel(V2_IMPLEMENTATION_STREAM.IDSRC),
		).toBe("Источники данных");
