...
describe("v2-implementation-streams.util", () => {
	it("exposes stable codes with stream labels (keys ≤ 6 chars)", () => {
		...
		expect(V2_IMPLEMENTATION_STREAM_LABELS.kmbkcb).toBe(
			"Разработка моделей КМБ и КСБ",
		);
		expect(V2_IMPLEMENTATION_STREAM_LABELS.rb).toBe("Моделирование РБ");
		expect(V2_IMPLEMENTATION_STREAM_LABELS.ptitpc).toBe(
			"AI-модели партнерств",
		);
		expect(V2_IMPLEMENTATION_STREAM_LABELS.finmdl).toBe(
		...
		expect(V2_IMPLEMENTATION_STREAM_LABELS.pirm).toBe(
	...
	it("validates codes and resolves labels", () => {
		...
		expect(resolveImplementationStreamLabel("idsrc")).toBe(
			"Источники данных",
		);
