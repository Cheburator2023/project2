...
export function binaryEnumKey(values: string[]): string {
	return values
...
export function isBinaryBooleanEnum(values: string[]): boolean {
...
export function coerceBinaryBooleanFormValue(value: unknown): boolean | undefined {
	if (typeof value === "boolean") return value;
	if (typeof value !== "string") return undefined;
	const normalized = value.trim().toLowerCase();
	if (POSITIVE_BINARY_LABELS.has(normalized)) return true;
	if (NEGATIVE_BINARY_LABELS.has(normalized)) return false;
	return undefined;
}

export function isPositiveBinaryFormValue(value: unknown): boolean {
	return coerceBinaryBooleanFormValue(value) === true;
}

/** Ключ в coefficientByField.values для boolean или legacy string. */
export function binaryEnumCoeffKey(
	value: unknown,
	positiveLabel: "Да" | "Требуется" = "Да",
): string | undefined {
	const coerced = coerceBinaryBooleanFormValue(value);
	if (coerced === true) return positiveLabel;
	if (coerced === false) {
