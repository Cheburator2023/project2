...
import {
	...
	V2_UI_OPTION_ARCH_BLOCK_UID,
	V2_UI_OPTION_ARCH_COMPONENT,
	V2_UI_OPTION_SCHEMA_FIELD_UID,
	V2_UI_OPTION_SEMANTIC_ROLE,
...
describe("buildV2SchemaFieldIndex", () => {
	it("indexes schemaFieldUid and archComponent after nested move-like paths", () => {
		...
		expect(resolveDotPathByArchComponent(index, "modelService")).toBe(
			"detailInfo.sectionA.modelService",
		);
		expect(index.byBlockUid.get("block-ms-1")?.dotPath).toBe(
			"detailInfo.sectionA.modelService",
		);
		expect(index.byUid.get("field-pilot-1")?.dotPath).toBe(
			"detailInfo.sectionA.modelService.pilot",
		);
	...
	it("resolves form values by uid and semantic role", () => {
		...
		expect(resolveFormValueByFieldUid(formData, index, "field-channels")).toBe(
			"Онлайн",
		);
		expect(
			resolveFormValueBySemanticRole(formData, index, "deploymentChannels"),
		).toBe("Онлайн");
	});
