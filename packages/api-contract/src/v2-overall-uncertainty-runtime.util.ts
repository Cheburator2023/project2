...
export function parseUncertaintyRiskFormEntry(
	value: unknown,
): V2UncertaintyRiskFormEntry | string | null {
	if (value == null || value === "") return null;
	...
	const probability =
		typeof value.probability === "string" ? value.probability.trim() : "";
	const goals = typeof value.goals === "string" ? value.goals.trim() : "";
	if (!probability && !goals) return null;
	return { probability, goals };
}

export function isStructuredUncertaintyRiskEntry(
	value: unknown,
): value is V2UncertaintyRiskFormEntry {
	const parsed = parseUncertaintyRiskFormEntry(value);
	return Boolean(parsed && typeof parsed === "object");
}

export function riskGroupHasStructuredEntries(
	riskGroup: Record<string, unknown> | undefined,
): boolean {
	if (!riskGroup) return false;
	return Object.values(riskGroup).some((value) =>
		isStructuredUncertaintyRiskEntry(value),
	);
}

/** Legacy: строка = имя группы («Низкий» / …). */
export function isLegacyUncertaintyGroupName(
	value: string,
 ...
 */
export function mapFormDataToOverallUncertaintyPreview(
	...
	if (riskGroup) {
		for (const [id, raw] of Object.entries(riskGroup)) {
			if (risks.some((risk) => risk.id === id)) continue;
			const parsed = parseUncertaintyRiskFormEntry(raw);
			if (!parsed || typeof parsed === "string") continue;
			if (!parsed.probability?.trim() || !parsed.goals?.trim()) continue;
			risks.push({
				id,
				enabled: true,
...
export function resolveOverallUncertaintyConfig(
 ...
 */
export function resolveLegacyUncertaintyCoefficientFromRiskGroupNames(
	formData: Record<string, unknown>,
	config: V2OverallUncertaintyConfig,
): { calculated: boolean; coefficient: number } | null {
	const uncertainty = readRecord(formData.uncertaintyCalculation);
	const riskGroup = readRecord(uncertainty?.riskGroup);
	if (!riskGroup || riskGroupHasStructuredEntries(riskGroup)) return null;
	...
	const riskSum = entries.reduce(
		(sum, name) => sum + (groupByName.get(name) ?? 0),
		0,
	);
	const coefficient =
		Math.round((1 + riskSum + (adjPct ?? 0) / 100) * 100) / 100;
