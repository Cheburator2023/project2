...
import {
	buildV2KeycloakGroupRoleTargetWithCompat,
	expandV2KeycloakTargetsWithCompat,
	mergeV2PermissionsWithImplied,
	normalizeV2UserGroupsWithCompat,
	resolveV2ImpliedPermissionsFromGroups,
} from "./v2-role-compat.util";
...
describe("normalizeV2UserGroupsWithCompat", () => {
	it("maps bare /admin_it to appadmin when toggle on", () => {
		expect(
			normalizeV2UserGroupsWithCompat(["/admin_it"], {
	...
	it("still maps AD leaf sum_appadmin under /admin_it", () => {
		expect(
			normalizeV2UserGroupsWithCompat(["/admin_it/test_sum_appadmin"], {
				adminItAsAppadmin: false,
			}),
...
describe("buildV2KeycloakGroupRoleTargetWithCompat", () => {
	it("adds nested lead paths when allowed", () => {
		...
		expect(target["/modelops/modelops_lead"]).toEqual(
			BASE_TARGET["/modelops_lead"],
		);
...
describe("expandV2KeycloakTargetsWithCompat", () => {
	it("duplicates lead roles onto nested paths", () => {
		const expanded = expandV2KeycloakTargetsWithCompat(
			BASE_TARGET,
			"test_",
			{ allowNestedLeadGroups: true },
		);
		expect(expanded["/de/de_lead"]).toEqual(
			expect.arrayContaining(["anketa_workflow_approve", "anketa_complete_anketa"]),
		);
		expect(expanded["/modelops/modelops_lead"]).toEqual(
...
describe("resolveV2ImpliedPermissionsFromGroups", () => {
	...
	it("returns empty when toggle off", () => {
		expect(
			resolveV2ImpliedPermissionsFromGroups(["/de/de_lead"], {
