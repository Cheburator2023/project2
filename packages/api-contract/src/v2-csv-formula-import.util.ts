...
function resolveArchCountFormulaToken(
	label: string,
): { kind: "model" | "sourceSystem"; token: string } | null {
...
function stripCoeffParamNamePrefix(raw: string): string {
	return clean(raw)
		.replace(/^[—–\-]+\s*/u, "")
		.replace(/^надбавка\s+за\s+/iu, "");
}

function stripCoeffValueLabelPrefix(raw: string): string {
	return clean(raw)
		.replace(/^\d+\s*[-–.]\s*/u, "")
		.replace(/^[×x*]\s*/u, "");
}

function parseCoeffArrowValue(raw: string): number | null {
	const text = clean(raw)
		// «0, 25» / «0. 25» → опечатка десятичного разделителя с пробелом
		.replace(/(\d)[.,]\s+(\d)/g, "$1.$2");
	if (!text) return null;
	const baseMul = text.match(
		/(?:^\+|баз[аы]|норматив).{0,20}?(\d+(?:[.,]\d+)?)/iu,
	);
	if (baseMul?.[1]) {
	...
	if (/^×?\s*1\b/u.test(text) || /^по\s+умолчанию/iu.test(text)) return 1;
	return null;
}
...
export function parseModelStreamLaborCoefficients(
	...
	const isShortValueLabel = (line: string) =>
		/^(да|нет|неизвестно|не\s+требуется)$/iu.test(line) ||
		/^далее\s/iu.test(line);
	const isParamHeaderLine = (line: string) => {
		if (!line || line.includes("→") || isShortValueLabel(line)) return false;
	...
	type CoeffBlock = { paramName: string; body: string };
	const coeffBlocks: CoeffBlock[] = [];
	let active: CoeffBlock | null = null;
	...
	for (const { paramName, body } of coeffBlocks) {
		if (!paramName || /^база/iu.test(paramName)) continue;
		if (!body.includes("→")) continue;
		if (isShortValueLabel(paramName) || paramName.length < 3) continue;
		...
		for (const match of body.matchAll(
			...
			if (!label || coefficient == null) continue;
		...
		for (const line of body.split(/\r?\n/)) {
			...
			if (/^далее\s/iu.test(text) || /^далее$/iu.test(text)) continue;
			if (/^[:—–\-]*$/u.test(text)) continue;
			const label = stripCoeffValueLabelPrefix(text);
			if (!label || values.some((row) => row.label === label)) continue;
		...
		const furtherMatch = body.match(
			/далее\s+везде\s*(-?\d+(?:[.,]\d+)?)/iu,
		);
		if (furtherMatch?.[1]) {
...
function parseLaborParamsNumbered(raw: string): string[] {
	...
	if (plainLines.length > 1) return plainLines;
	if (trimmed.includes(";")) {
		return trimmed
			.split(";")
			.map((part) => clean(part))
...
export function parseCsvTriggerRules(raw: string): CsvFormulaTriggerRule[] {
	return splitTopLevelList(raw)
		...
		.map((rawParam): CsvFormulaTriggerRule => {
		...
		if (control?.[1] && control[2]) {
			const mapped = mapCsvControlTypeValue(
				clean(control[2]).replace(/^[«"]|[»"]$/gu, ""),
			);
			return {
...
export function parseCsvFormulaImportRows(
	...
	const idx = (prefix: string) =>
		header.findIndex((h) =>
			normalizeCsvHeader(h).startsWith(normalizeCsvHeader(prefix)),
		);
	...
	const cName = header.findIndex((h) =>
		normalizeCsvHeader(h).includes("название в смарт-анкете"),
	);
	...
	return rows
		...
		.map((r): CsvFormulaImportRow => {
				...
				laborFromCoeffs.length > 0
					? [
							...
							...laborFromCol.filter((name) => {
								...
								if (
									laborFromCoeffs.some(
										(coeffName) =>
											normalizeParamLabel(coeffName) === norm,
									)
								) {
									return false;
								}
								...
								if (
									laborFromCoeffs.some((coeffName) => {
			...
			if (
				/[KК]\s*\(\s*кол-?во\s+моделей/iu.test(formulaRaw) &&
				!laborFromCoeffs.some((name) =>
					normalizeParamLabel(name).includes("моделей"),
				)
			) {
				const fill =
					"\nКоличество моделей:\n1→1\n2→1\nДалее везде 1\n";
...
const CSV_PARAM_LABEL_ALIASES: Record<string, string> = {
...
export function isCsvModelServiceCreateTrigger(paramName: string): boolean {
	return normalizeParamLabel(paramName).includes("создание модельного сервиса");
}

const CONTROL_TYPE_VALUE_LABELS: Record<string, string> = {
...
export function mapCsvControlTypeValue(raw: string): string {
...
function fixMisplacedExcelRoundClose(flat: string): string {
	// ОКРУГЛ.ВВЕРХ(expr); 2) → ОКРУГЛ.ВВЕРХ(expr; 2), с балансом скобок в expr.
	const kinds = ["ОКРУГЛ.ВВЕРХ", "ОКРУГЛ.ВНИЗ", "ОКРУГЛ"] as const;
...
export function extractFormulaCoreFromCsvText(formulaRaw: string): {
	...
	const flat = fixMisplacedExcelRoundClose(
		formulaRaw.replace(/\r/g, "").replace(/\n/g, " "),
	);
...
function normalizeFormulaCoreAliases(core: string): string {
...
function extractBracketCoeffLabel(raw: string): string | null {
	const text = clean(raw);
	const bracket = text.match(/^[KК]\s*\[\s*(.+?)\s*\]$/iu);
	if (bracket?.[1]) {
		return clean(bracket[1]).replace(/\s*=\s*.+$/u, "").trim() || null;
	}
	if (!/^[KК]\s*\(/iu.test(text)) return null;
...
> {
	...
	const resolveToken = (
		label: string,
		bucket: string[] = unmatchedParams,
	) => resolveCoeffOrArchToken(label, resolveParamCode, bucket, preferLaborLabels);
	...
	const additiveParen = normalized.match(
		/^Норматив\s*×\s*\((.+)\)$/iu,
	);
	...
	if (/\+\s*\(/u.test(normalized) && /\(Норматив\s*×/iu.test(normalized)) {
		const terms = [...normalized.matchAll(/\(([^)]+)\)/gu)].map((m) =>
			clean(m[1] ?? ""),
		);
		const tokens: string[] = [];
		const localUnmatched: string[] = [];
		for (const term of terms) {
			...
			const label =
				extractBracketCoeffLabel(operand) ??
	...
	if (/[KК]\s*[\[(]/iu.test(normalized)) {
		return buildModelStreamFormulaTextFromCore(
			normalized,
			resolveParamCode,
			preferLaborLabels,
		);
...
> {
	...
	const operands = splitModelStreamFormulaOperands(
		normalizeFormulaCoreAliases(core)
			.replace(/^\d+(?:[.,]\d+)?\s*×\s*/u, "")
	...
	for (const operand of operands) {
		...
		const token = resolveToken(label);
		if (!token) {
			if (!unmatchedParams.includes(operand)) unmatchedParams.push(operand);
			continue;
		}
		if (pendingOp === "/") {
...
export function buildFormulaFromCsvRow(
	row: Pick<
		CsvFormulaImportRow,
		"formulaRaw" | "laborParams" | "laborCoefficientsRaw"
	>,
	candidates: CsvFormulaParamCandidate[],
	overrides: Record<string, string> = {},
	...
	const preferLaborLabels = new Set(
	...
	const built = buildFormulaTextFromCsvCore(
		extracted.core,
		resolve,
		preferLaborLabels,
	);
...
export function csvRowToCatalogPatch(
	...
	const explicitCoeffParams = new Set(
		parseModelStreamLaborCoefficients(row.laborCoefficientsRaw ?? "").map(
			(group) => normalizeParamLabel(group.paramName),
		),
	);
	const laborParams = (
		...
		if (explicitCoeffParams.has(norm)) return true;
	...
	const allowedParamCodes = new Set(
		laborParams.flatMap((paramName) => {
			...
			const aliasSlug = slugParamCode(
				CSV_PARAM_LABEL_ALIASES[normalizeParamLabel(paramName)] ?? paramName,
			);
