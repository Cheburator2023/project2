...
exports.V2_PER_INSTANCE_ARCH_COUNT_SHARE_KEY = exports.V2_PER_INSTANCE_ARCH_COUNT_OVERRIDE_KEY = void 0;
exports.resolveArchInstanceNameFieldKeys = resolveArchInstanceNameFieldKeys;
exports.resolveArchComponentKindFromType = resolveArchComponentKindFromType;
exports.resolveModelServiceSourceLabel = resolveModelServiceSourceLabel;
exports.listArchComponentInstances = listArchComponentInstances;
exports.readPerInstanceArchCountOverride = readPerInstanceArchCountOverride;
exports.readPerInstanceArchCountShare = readPerInstanceArchCountShare;
...
exports.formatNoTriggerMatchingArchInstanceBreakdown = formatNoTriggerMatchingArchInstanceBreakdown;
exports.mergeArchInstanceTriggerSource = mergeArchInstanceTriggerSource;
exports.archInstanceMatchesWorkTrigger = archInstanceMatchesWorkTrigger;
exports.matchTypicalWorkAppearanceTriggers = matchTypicalWorkAppearanceTriggers;
...
const v2_typical_works_util_2 = require("./v2-typical-works.util");
/** Маркер в formData: принудительный count для arch_count_coeff в per-instance режиме. */
exports.V2_PER_INSTANCE_ARCH_COUNT_OVERRIDE_KEY = "__v2PerInstanceArchCountOverride";
/**
 ...
 */
exports.V2_PER_INSTANCE_ARCH_COUNT_SHARE_KEY = "__v2PerInstanceArchCountShare";
 ...
 */
function resolveArchInstanceNameFieldKeys(schemaParams, kind) {
    if (!kind)
        return [];
 ...
 */
function resolveArchComponentKindFromType(archComponentType) {
    ...
    for (const kind of kinds) {
        if (kind.toLowerCase() === lower)
            return kind;
        const label = v2_anketa_section_ui_util_1.V2_ARCH_COMPONENT_LABELS[kind].toLowerCase();
        if (label === lower)
            return kind;
    }
...
function listSourceSystemRows(formData) {
    ...
    return rows.filter((item) => item != null &&
        typeof item === "object" &&
        !Array.isArray(item) &&
        (0, v2_typical_works_util_2.isFilledTypicalWorkSourceRow)(item));
}
function listDataMartRows(formData) {
    ...
    for (const candidate of candidates) {
        const rows = (0, v2_typical_works_util_1.readFilledArchComponentListRows)(candidate);
        if (rows.length > 0)
            return rows;
    }
    return [];
...
function readModelServiceRow(formData) {
    ...
    if (asRecord && (0, v2_typical_works_util_2.isFilledTypicalWorkSourceRow)(asRecord))
        return asRecord;
    return undefined;
}
/**
 ...
 */
function resolveModelServiceSourceLabel(formData, options) {
 ...
 */
function listArchComponentInstances(formData, archComponentType, options) {
...
function readPerInstanceArchCountOverride(formData, kind) {
    const raw = formData?.[exports.V2_PER_INSTANCE_ARCH_COUNT_OVERRIDE_KEY];
...
function readPerInstanceArchCountShare(formData, kind) {
    const record = readRecord(formData?.[exports.V2_PER_INSTANCE_ARCH_COUNT_SHARE_KEY]);
    if (!record)
        return null;
    const value = Number(record[kind]);
...
function withPerInstanceArchCountOverride(formData, kind, count = 1, shareOf) {
    if (!kind)
        return formData;
    const next = {
        ...formData,
        [exports.V2_PER_INSTANCE_ARCH_COUNT_OVERRIDE_KEY]: {
            ...readRecord(formData[exports.V2_PER_INSTANCE_ARCH_COUNT_OVERRIDE_KEY]),
            [kind]: count,
        },
    };
    if (shareOf != null && Number.isFinite(shareOf) && shareOf >= 1) {
        next[exports.V2_PER_INSTANCE_ARCH_COUNT_SHARE_KEY] = {
            ...readRecord(formData[exports.V2_PER_INSTANCE_ARCH_COUNT_SHARE_KEY]),
            [kind]: shareOf,
        };
    }
    return next;
 ...
 */
function formDataWithSingleArchInstance(formData, kind, instance, 
 ...
 */
function formatNoTriggerMatchingArchInstanceBreakdown(archComponentType) {
    const kind = resolveArchComponentKindFromType(archComponentType);
    const label = (kind ? v2_anketa_section_ui_util_1.V2_ARCH_COMPONENT_LABELS[kind] : null) ||
        archComponentType?.trim() ||
        "арх. компонент";
 ...
 */
function mergeArchInstanceTriggerSource(kind, baseSource, instanceRow) {
    if (kind === "model") {
        const merged = { ...baseSource };
        for (const [key, value] of Object.entries(instanceRow)) {
            if (!isMeaningfulArchInstanceOverlayValue(value))
                continue;
            merged[key] = value;
        }
        return merged;
...
function isMeaningfulArchInstanceOverlayValue(value) {
 ...
 */
function archInstanceMatchesWorkTrigger(params) {
    if (!(0, v2_trigger_formula_util_1.hasTypicalWorkTriggersConfigured)(params.triggerInput))
        return true;
    return (0, v2_trigger_formula_util_1.matchTypicalWorkTriggers)(params.triggerInput, params.source, params.formData, params.matchContext);
}
/**
 ...
 */
function matchTypicalWorkAppearanceTriggers(params) {
    if (!(0, v2_trigger_formula_util_1.hasTypicalWorkTriggersConfigured)(params.triggerInput))
        return false;
    ...
    if (kind != null && kind !== "modelService") {
        ...
        if (instances.length > 0) {
            return instances.some((instance) => archInstanceMatchesWorkTrigger({
                ...
                source: mergeArchInstanceTriggerSource(kind, params.source, instance.row),
