...
exports.METRICS_COUNT_LABOR_RANGES = void 0;
exports.buildProductionAdditionalVitrinsLaborRows = buildProductionAdditionalVitrinsLaborRows;
exports.resolveNumericLaborPresetRows = resolveNumericLaborPresetRows;
exports.isNumericLaborByValueParam = isNumericLaborByValueParam;
exports.buildNumericLaborCoefficientRows = buildNumericLaborCoefficientRows;
exports.METRICS_COUNT_LABOR_RANGES = [
...
function resolveNumericLaborPresetRows(paramName) {
    const title = normalizeParamTitle(paramName ?? "");
    if (!title)
        return null;
    if (title.includes("количество метрик")) {
        return exports.METRICS_COUNT_LABOR_RANGES;
    }
    if (title.includes("необходимость продуктивизации") &&
...
function isNumericLaborByValueParam(input) {
    if (resolveNumericLaborPresetRows(input.name ?? null))
        return true;
    return Boolean(input.numeric) && (input.values?.length ?? 0) === 0;
}
function buildNumericLaborCoefficientRows(preset, ctx) {
