...
function normalizePublishArchComponent(raw) {
...
function buildFactoryCatalogRowKey(row) {
    return [
...
function cardToCatalogRow(card, coverageDate, dropped) {
    ...
    const formulaText = card.formula.text?.trim() ||
        (0, v2_work_formula_util_1.tokensToText)(card.formula.tokens) ||
        "N";
 ...
 */
function publishFactoryTypicalWorksBundle(input) {
...
function rebuildFactoryCatalogSnapshotMeta(typicalWorks, previous) {
    const streams = [
