...
(0, vitest_1.describe)("reconcileTypicalWorkCardWithSchemaField", () => {
     ...
     */
    (0, vitest_1.it)("идемпотентна: повторная синхронизация не помечает карточку изменённой", () => {
        const request = {
        ...
        const first = (0, v2_typical_work_schema_sync_util_1.reconcileTypicalWorkCardWithSchemaField)(card(), request);
        (0, vitest_1.expect)(first.changed).toBe(true);
        const second = (0, v2_typical_work_schema_sync_util_1.reconcileTypicalWorkCardWithSchemaField)(first.card, request);
    ...
    (0, vitest_1.it)("bulk dryRun: уже актуальная привязка с @-алиасами не трогает формулу", () => {
        const bound = card();
        bound.rules = [];
        bound.laborParams = [
            {
        ...
        (0, vitest_1.expect)(result.changed).toBe(false);
        (0, vitest_1.expect)(result.card).toEqual(bound);
    });
    (0, vitest_1.it)("восстанавливает обрезанный paramName и дальше идемпотентна", () => {
        const bound = card();
        bound.rules = [];
        bound.laborParams = [
            {
        ...
        const first = (0, v2_typical_work_schema_sync_util_1.reconcileTypicalWorkCardWithSchemaField)(bound, request);
        (0, vitest_1.expect)(first.changed).toBe(true);
        const repairedName = first.card.laborParams[0]?.paramName ?? "";
        (0, vitest_1.expect)(repairedName).toContain("Применение модельного сервиса в разных контурах (region и inno.local)");
        (0, vitest_1.expect)(repairedName.startsWith(" @ ")).toBe(false);
        const second = (0, v2_typical_work_schema_sync_util_1.reconcileTypicalWorkCardWithSchemaField)(first.card, request);
        (0, vitest_1.expect)(second.changed).toBe(false);
        (0, vitest_1.expect)(second.card).toEqual(first.card);
    });
    (0, vitest_1.it)("bulk binding sync: префикс «Маркер:» / регистр не считаются изменением схемы", () => {
        const bound = card();
        bound.rules = [
            {
                id: "rule-marker",
        ...
        (0, vitest_1.expect)(result.changed).toBe(false);
        (0, vitest_1.expect)(result.card.rules[0]?.paramName).toBe("Требуется разметка данных источника");
    });
    ...
    (0, vitest_1.it)("marks formula tokens invalid when matching labor param is deleted", () => {
        const legacy = card();
        legacy.rules = [];
        legacy.laborParams = [
            {
