...
function toBoardData(rows) {
    const byStatus = new Map();
    for (const status of kanban_board_types_1.KANBAN_BOARD_STATUSES) {
    ...
    const board = {
        root: {
            ...
            title: "Root",
            parentId: null,
            children: kanban_board_types_1.KANBAN_BOARD_STATUSES.map((status) => status.id),
            totalChildrenCount: kanban_board_types_1.KANBAN_BOARD_STATUSES.length,
        },
    };
    for (const status of kanban_board_types_1.KANBAN_BOARD_STATUSES) {
        ...
        board[status.id] = {
            ...
            content: { color: kanban_board_types_1.KANBAN_BOARD_COLUMN_COLORS[status.id] },
        };
