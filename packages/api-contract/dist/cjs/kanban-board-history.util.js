...
exports.KANBAN_BOARD_HISTORY_OVERVIEW_PREVIEW_LIMIT = exports.KANBAN_BOARD_TASK_HISTORY_MAX_PER_TASK = void 0;
exports.formatKanbanBoardHistoryValue = formatKanbanBoardHistoryValue;
exports.kanbanBoardTaskHistorySnapshot = kanbanBoardTaskHistorySnapshot;
exports.diffKanbanTaskChanges = diffKanbanTaskChanges;
const kanban_board_types_1 = require("./kanban-board.types");
exports.KANBAN_BOARD_TASK_HISTORY_MAX_PER_TASK = 6;
/** Сколько последних изменений показывать на общей странице истории по доске. */
exports.KANBAN_BOARD_HISTORY_OVERVIEW_PREVIEW_LIMIT = 4;
...
const isSubtaskItem = (value) => isRecord(value) && typeof value.text === "string";
const formatSubtasks = (value) => {
    ...
    return value
        ...
        const status = item.status
            ? (0, kanban_board_types_1.kanbanBoardSubtaskStatusTitle)(item.status)
            : (0, kanban_board_types_1.kanbanBoardSubtaskIsDone)(item)
                ? "Готово"
                : "В работе";
...
const formatRoleEstimates = (value) => {
    if (!isRecord(value))
        return null;
    const entries = Object.entries(value).filter(([, amount]) => amount != null && amount !== 0);
    if (!entries.length)
        return null;
    return entries.map(([role, amount]) => `${role}: ${amount}`).join(", ");
};
const formatPlainObject = (value) => {
...
function formatKanbanBoardHistoryValue(value) {
...
const CONTENT_FIELDS = [
    ...
    { key: "description", label: "Описание" },
    {
        key: "priority",
        label: "Приоритет",
        format: (value) => (0, kanban_board_types_1.kanbanBoardPriorityTitle)(String(value ?? "")),
    },
    ...
    { key: "currentAssignee", label: "Текущий исполнитель" },
    {
        key: "taskType",
        label: "Тип задачи",
        format: (value) => (0, kanban_board_types_1.kanbanBoardTaskTypeTitle)(String(value ?? "")),
    },
    {
        key: "workType",
        label: "Тип работ",
        format: (value) => (0, kanban_board_types_1.kanbanBoardWorkTypeTitle)(String(value ?? "")),
    },
...
function kanbanBoardTaskHistorySnapshot(input) {
    return {
...
function diffKanbanTaskChanges(before, after, labels) {
    const changes = [];
    const columnTitle = labels?.columnTitle ?? ((id) => id);
