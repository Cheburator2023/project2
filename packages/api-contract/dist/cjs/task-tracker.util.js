...
const task_tracker_types_1 = require("./task-tracker.types");
function toBoardData(rows) {
    const byStatus = new Map();
    for (const status of task_tracker_types_1.TASK_STATUSES) {
    ...
    const board = {
        root: {
            ...
            title: "Root",
            parentId: null,
            children: task_tracker_types_1.TASK_STATUSES.map((status) => status.id),
            totalChildrenCount: task_tracker_types_1.TASK_STATUSES.length,
        },
    };
    for (const status of task_tracker_types_1.TASK_STATUSES) {
...
function fromBoardData(board, stand, now) {
    const out = [];
    for (const status of task_tracker_types_1.TASK_STATUSES) {
