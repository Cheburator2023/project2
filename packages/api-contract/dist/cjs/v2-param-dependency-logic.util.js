...
exports.V2_PARAM_DEPENDENCY_RULE_ID_PREFIX = void 0;
exports.isParamDependencyLogicRule = isParamDependencyLogicRule;
exports.parseParamDependencyGraphFromLogic = parseParamDependencyGraphFromLogic;
exports.buildParamDependencyVisibilityRules = buildParamDependencyVisibilityRules;
exports.mergeParamDependencyRulesIntoLogic = mergeParamDependencyRulesIntoLogic;
exports.evaluateParamDependencyRulesForSource = evaluateParamDependencyRulesForSource;
exports.resolveHiddenParamCodesForSource = resolveHiddenParamCodesForSource;
exports.resolveHiddenSourceFieldKeys = resolveHiddenSourceFieldKeys;
exports.filterCoefficientLogicForHiddenFields = filterCoefficientLogicForHiddenFields;
exports.buildPerRowItemVisibilityCondition = buildPerRowItemVisibilityCondition;
...
exports.V2_PARAM_DEPENDENCY_RULE_ID_PREFIX = "param-dep-";
...
function ruleIdFor(targetParamCode, pointer) {
    const pointerSlug = pointer
        ...
        .slice(0, 48);
    return `${exports.V2_PARAM_DEPENDENCY_RULE_ID_PREFIX}${targetParamCode}__${pointerSlug}`;
}
...
function isParamDependencyLogicRule(rule) {
    if (rule.id.startsWith(exports.V2_PARAM_DEPENDENCY_RULE_ID_PREFIX))
        return true;
    const payload = rule.payload;
    return payload?.paramDependency === true;
}
function parseParamDependencyGraphFromLogic(rules) {
...
function buildSingleCondition(condition, sourcePointers, targetPointer) {
    ...
    const varPath = (0, v2_logic_rule_builders_util_1.v2JsonPointerToVarPath)(sourcePointer).replace(/\.items\./g, ".0.");
...
function buildParamDependencyVisibilityRules(graph, bindings) {
...
function mergeParamDependencyRulesIntoLogic(rules, graph, bindings) {
...
function evaluateParamDependencyRulesForSource(rules, source, paramDefs) {
    if (rules.length === 0)
        return true;
    ...
    return rules.every((rule) => {
        ...
        const actual = (0, v2_works_catalog_match_util_1.readTypicalWorkSourceField)(source, rule.sourceParamCode, paramName);
        const expected = rule.valueLabel.trim() || rule.valueCode;
        return compareDependencyValue(actual, expected, rule.operator);
    });
}
function resolveHiddenParamCodesForSource(graph, source, paramDefs) {
...
function resolveHiddenSourceFieldKeys(hiddenParamCodes, paramDefs, source) {
    const keys = new Set();
...
function filterCoefficientLogicForHiddenFields(logic, hiddenSourceFields) {
    if (!logic || typeof logic !== "object" || Array.isArray(logic))
        return logic;
...
function buildPerRowItemVisibilityCondition(rules, sourcePointers, targetPointer) {
    const targetItems = itemsFieldInfo(targetPointer);
    const sourcePointer = sourcePointers[0];
