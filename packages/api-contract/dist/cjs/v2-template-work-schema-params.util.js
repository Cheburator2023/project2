...
function resolveFieldArchComponent(uiSchema, segments) {
    for (let length = segments.length; length >= 0; length--) {
        ...
        if (arch)
            return v2_anketa_section_ui_util_1.V2_ARCH_COMPONENT_LABELS[arch];
    }
    return null;
}
...
function buildWorkSchemaParamsFromTemplate(params) {
    const out = [];
...
function collectTypicalWorkSchemaConsistencyIssues(input) {
    const issues = [];
    const seen = new Set();
    ...
    for (const labor of input.laborParamCodes) {
        const resolved = (0, v2_work_schema_params_match_util_1.resolveWorkSchemaParamForRule)(labor, input.schemaParams);
        if (!resolved) {
    ...
    if (input.formulaTokens && input.formulaTokens.length > 0) {
        const formulaError = (0, v2_work_formula_util_1.validateWorkFormulaTokens)(input.formulaTokens, {
...
function findCatalogPreviousCodeForSchemaParam(catalog, schemaParam) {
    const normSchemaName = (0, v2_work_param_source_keys_util_1.stripParamNameSourceKeys)(schemaParam.name)
        .trim()
        .toLowerCase();
