...
function buildLaborCoefficientLookupSource(source, formData, schemaParams, paramCodes) {
    ...
    for (const code of codes) {
        ...
        const targetName = (0, v2_work_param_source_keys_util_1.stripParamNameSourceKeys)(schemaParams.find((param) => param.code === code)?.name ?? "")
            .trim()
            .toLowerCase();
        if (targetName) {
            for (const alias of schemaParams) {
                if (alias.code === code)
                    continue;
                if ((0, v2_work_param_source_keys_util_1.stripParamNameSourceKeys)(alias.name).trim().toLowerCase() !==
                    targetName) {
                    continue;
                }
        ...
        for (const [key, raw] of Object.entries(source)) {
            ...
            if (!sp)
                continue;
            const spName = (0, v2_work_param_source_keys_util_1.stripParamNameSourceKeys)(sp.name).trim().toLowerCase();
            if (!spName)
                continue;
