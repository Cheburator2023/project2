...
import { buildParamDependencyVisibilityRules, evaluateParamDependencyRulesForSource, filterCoefficientLogicForHiddenFields, isParamDependencyLogicRule, mergeParamDependencyRulesIntoLogic, parseParamDependencyGraphFromLogic, resolveHiddenParamCodesForSource, resolveHiddenSourceFieldKeys, } from "./v2-param-dependency-logic.util";
const bindings = [
    {
        paramCode: "source",
        paramName: "Источник",
...
describe("v2-param-dependency-logic.util", () => {
    ...
    it("resolves hidden param codes from source row", () => {
        ...
        const hidden = resolveHiddenParamCodesForSource(graph, { type: "Внешний" }, defs);
        expect(hidden.has("slozhnost")).toBe(true);
        expect(evaluateParamDependencyRulesForSource(graph.targets[0].rules, { type: "Внутренний" }, defs)).toBe(true);
    });
    it("filters coefficient logic for hidden source fields", () => {
        ...
        const filtered = filterCoefficientLogicForHiddenFields(logic, new Set(["domainComplexity"]));
        expect(filtered["*"][0]).toBe(1);
        expect(filtered["*"][1]).toEqual(logic["*"][1]);
    });
    it("maps hidden param codes to source field keys", () => {
        const keys = resolveHiddenSourceFieldKeys(new Set(["slozhnost_predmetnoj_oblasti"]), [
            {
