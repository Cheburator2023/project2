...
import { normalizeV2AnketaWorkflow, } from "./v2-anketa-workflow.util";
...
import { normalizeV2UserGroups, resolveV2UserImplementationStreamsFromGroups, resolveV2UserScopedStreamsFromGroups, } from "./v2-user-stream-mapping.util";
 ...
 */
export const V2_ANKETA_EDIT_ONLY_OWN_STREAM_ROLE_CODES = ["sarep"];
/**
...
export function userEditsOnlyOwnStreamBlocks(roles) {
    return roles.some((role) => V2_ANKETA_EDIT_ONLY_OWN_STREAM_ROLE_CODES.includes(role.trim()));
}
/**
...
export function isSharedAnketaSectionPath(formPath) {
    ...
    return V2_ANKETA_SHARED_SECTION_KEYS.includes(root);
}
 ...
 */
export function isV2AnketaPathEditableForViewer(viewer, uiSchema, formPath, options) {
    if (options?.applyAccessRules === false || !viewer)
        return true;
 ...
 */
export function canViewerCompleteAnketaSection(viewer, uiSchema, sectionPath, options) {
    if (options?.applyAccessRules === false || !viewer)
        return true;
    if (!userEditsOnlyOwnStreamBlocks(viewer.roles))
        return true;
...
export function canViewerCompleteWholeAnketa(roles) {
 ...
 */
export function collectForbiddenV2AnketaWorkflowChanges(viewer, uiSchema, previous, next) {
