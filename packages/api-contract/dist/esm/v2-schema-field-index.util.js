 ...
 */
export function buildV2SchemaFieldIndex(jsonSchema, uiSchema) {
    const byUid = new Map();
...
export function resolveDotPathByArchComponent(index, archComponent, fallbackDotPath) {
    const fromIndex = index.byArchComponent.get(archComponent)?.dotPath;
    if (fromIndex)
        return fromIndex;
    return fallbackDotPath?.trim() || null;
}
export function resolveDotPathByBlockUid(index, blockUid, fallbackDotPath) {
    const fromIndex = index.byBlockUid.get(blockUid)?.dotPath;
    if (fromIndex)
        return fromIndex;
    return fallbackDotPath?.trim() || null;
}
export function resolvePointerByFieldUid(index, schemaFieldUid) {
    return index.byUid.get(schemaFieldUid.trim())?.pointer ?? null;
}
export function resolveDotPathByFieldUid(index, schemaFieldUid) {
    return index.byUid.get(schemaFieldUid.trim())?.dotPath ?? null;
}
export function resolveFieldUidBySemanticRole(index, role) {
    return index.byRole.get(role.trim()) ?? null;
}
/** Читает formData по schema pointer (`/a/b/items/c`). */
export function readFormValueAtSchemaPointer(root, pointer) {
...
export function readFormValueAtDotPath(root, dotPath) {
...
export function resolveFormValueByFieldUid(formData, index, schemaFieldUid) {
    const pointer = resolvePointerByFieldUid(index, schemaFieldUid);
    if (!pointer)
        return undefined;
    return readFormValueAtSchemaPointer(formData, pointer);
}
export function resolveFormValueBySemanticRole(formData, index, role) {
...
export function schemaPointerToDotPath(pointer) {
    return pointer
        .split("/")
        .filter(Boolean)
 ...
 */
export function rewriteSchemaPathsInValue(value, oldPointer, newPointer) {
