...
import { buildLaborCoefficientLookupSource, coerceNumericLaborActual, findFieldValueInFormData, flattenSourceContextValue, laborValueMatches, resolveByValueLaborParamCoefficientDetails, resolveByValueLaborParamCoefficients, } from "./v2-works-catalog-match.util";
describe("numeric labor coefficient ranges", () => {
    ...
    it("resolves multi-model readyPromReports with max and source labels", () => {
        ...
        const lookup = buildLaborCoefficientLookupSource({}, formData, [], ["readyPromReports"]);
        expect(lookup.readyPromReports).toEqual([true, false]);
        const details = resolveByValueLaborParamCoefficientDetails(lookup, [
            {
                paramCode: "readyPromReports",
                paramName: "Наличие готовых промышленных витрин",
                valueCode: "Да",
