...
export const V2_TYPICAL_WORK_ALWAYS_TRIGGER_PARAM_NAME = "Нет — работа выводится всегда";
const ALWAYS_TRIGGER_NAME_RE = /нет\s*[—–-]\s*работа\s+выводится\s+всегда/iu;
export function isAlwaysShownTriggerParam(paramCode, paramName) {
    const code = paramCode.trim().toLowerCase();
    if (code === V2_TYPICAL_WORK_ALWAYS_TRIGGER_PARAM_CODE ||
 ...
 */
export function schemaPointerFieldParentExists(root, pointer) {
    if (!pointer.startsWith("/"))
        return false;
    const segments = pointer.split("/").filter(Boolean);
 ...
 */
export function overlayCrossComponentTriggerLookup(lookup, source, formData, paramCodes) {
...
export function findFieldValuesWithSourceLabels(formData, fieldCode) {
...
export function buildLaborCoefficientLookupSource(source, formData, schemaParams, paramCodes, options) {
    const merged = { ...source };
    ...
    for (const code of codes) {
        ...
        const targetName = stripParamNameSourceKeys(schemaParams.find((param) => param.code === code)?.name ?? "")
            .trim()
            .toLowerCase();
        if (targetName) {
            for (const alias of schemaParams) {
                if (alias.code === code)
                    continue;
                if (stripParamNameSourceKeys(alias.name).trim().toLowerCase() !==
                    targetName) {
                    continue;
                }
        ...
        for (const [key, raw] of Object.entries(source)) {
            ...
            const isWorkTypeAlias = spName === "тип работ" &&
...
export function typicalWorkRulesMatchSource(rules, source, formData, triggerArchCount, matchContext) {
    ...
    const schemaFieldIndex = matchContext?.schemaFieldIndex ??
        (matchContext?.jsonSchema || matchContext?.uiSchema
            ? buildV2SchemaFieldIndex(matchContext.jsonSchema, matchContext.uiSchema)
            : null);
 ...
 */
export function resolveByValueLaborParamCoefficientDetails(source, rows, _formData) {
    const details = {};
...
export function resolveByValueLaborParamCoefficients(source, rows, _formData) {
