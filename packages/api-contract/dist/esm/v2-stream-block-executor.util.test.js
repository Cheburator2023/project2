...
describe("v2-stream-block-executor.util", () => {
    ...
    it("maps legacy block keys to implementation stream codes", () => {
        ...
        expect(inferLegacyStreamBlockExecutorCode("streamModelControl")).toBe(V2_IMPLEMENTATION_STREAM.MDLCTL);
    });
    it("resolves labels and db scopes for typical work matching", () => {
        expect(resolveStreamBlockExecutorLabel(V2_IMPLEMENTATION_STREAM.IDSRC)).toBe(V2_IMPLEMENTATION_STREAM_LABELS[V2_IMPLEMENTATION_STREAM.IDSRC]);
        expect(resolveStreamBlockExecutorScopeStreams(V2_IMPLEMENTATION_STREAM.IDSRC)).toEqual(expect.arrayContaining([
            "ИД. Внутренний",
            "Источники данных",
            V2_IMPLEMENTATION_STREAM.IDSRC,
        ]));
        expect(resolveStreamBlockExecutorScopeStreams(V2_IMPLEMENTATION_STREAM.KMBKCB)).toEqual(expect.arrayContaining([
