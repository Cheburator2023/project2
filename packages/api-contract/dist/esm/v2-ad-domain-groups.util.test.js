...
import { expandV2KeycloakTargetsWithAdAliases, mapV2AdGroupLeafToRoleCodes, normalizeV2AdStandPrefix, stripV2AdStandPrefix, } from "./v2-ad-domain-groups.util";
...
describe("v2-ad-domain-groups", () => {
    ...
    it("expands sync TARGET with AD aliases", () => {
        ...
        expect(expanded["/test_sum_appadmin"]).toEqual(expect.arrayContaining(["anketa_audit_view"]));
        expect(expanded["/test_sum_sacfg"]).toEqual(expect.arrayContaining(["anketa_view_all_calculations"]));
        const noStand = expandV2KeycloakTargetsWithAdAliases(target, "");
        expect(noStand["/sum_appadmin"]).toEqual(expect.arrayContaining(["anketa_audit_view"]));
    });
});
