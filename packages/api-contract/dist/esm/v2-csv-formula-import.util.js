...
export function parseNormFromCsv(raw) {
...
export function normalizeCsvArchComponent(raw) {
    const v = clean(raw)
...
export function inferCsvArchComponent(rawComponent, stream, stage) {
...
export function stripWorkStagePrefix(name) {
    let rest = name.trim();
...
export function extractE2eWorkStage(name) {
...
export function resolveE2eWorkStageAndName(smartName) {
...
export function buildLinearArchCountSteps(maxCount, increment = 0.75) {
    const steps = [];
...
export const MODEL_STREAM_SOURCE_COUNT_STEPS = [
...
export function formatArchCountFormulaSteps(steps) {
    return steps
...
export function parseModelStreamLaborCoefficients(raw) {
...
export function parseModelStreamTriggerRules(raw) {
...
export function parseCsvTriggerRules(raw) {
...
export function parseCsvLaborCoefficients(formulaRaw, archComponent) {
...
export function parseCsvFormulaImportRows(csvText) {
    ...
    return rows
        ...
        return {
            ...
            laborCoefficientsRaw: cCoeffs >= 0 ? (r[cCoeffs] ?? "") : undefined,
        };
    })
        .filter((row) => row.name.length > 0 && row.stream.length > 0);
}
export function buildCatalogMatchKey(row) {
    return [row.stream, row.component, row.stage, row.name]
        .map((part) => clean(part))
        .join("|");
}
export function normalizeParamLabel(label) {
    return label
        .toLowerCase()
        .replace(/ё/g, "е")
        .replace(/[«»""]/g, "")
...
export function resolveCsvParamCode(label, candidates, overrides = {}) {
...
export function extractFormulaCoreFromCsvText(formulaRaw) {
...
export function buildFormulaTextFromCsvCore(core, resolveParamCode) {
...
function buildModelStreamFormulaTextFromCore(core, resolveParamCode) {
    const unmatchedParams = [];
    const skippedSpecial = [];
    const operands = splitModelStreamFormulaOperands(core.replace(/^\d+(?:[.,]\d+)?\s*×\s*/u, "").replace(/^Норматив\s*×\s*/iu, ""));
    const exprParts = ["N"];
    let pendingOp = "*";
...
export function buildFormulaFromCsvRow(row, candidates, overrides = {}) {
...
export function csvRowToCatalogPatch(row, candidates, overrides = {}) {
