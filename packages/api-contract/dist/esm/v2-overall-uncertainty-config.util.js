import { INFLUENCE_VALUES, INITIATIVE_COST_VALUES, INITIATIVE_TIMELINE_VALUES, PROBABILITY_VALUES, } from "./calculation.constants";
import { V2_UNCERTAINTY_RISK_GROUP_LABELS, V2_UNCERTAINTY_RISK_GROUP_ORDER, } from "./v2-questionnaire-registry-columns.util";
export const V2_OVERALL_UNCERTAINTY_CONFIG_RULE_ID = "v2-overall-uncertainty-config";
export const V2_OVERALL_UNCERTAINTY_CONFIG_PAYLOAD_ROLE = "overall_uncertainty_config";
export const V2_UNCERTAINTY_ADJUSTMENT_DEFAULTS = {
    minPct: 0,
    maxPct: 30,
    defaultPct: 0,
...
export function isUncertaintyNotApplicableLabel(label) {
...
export const V2_UNCERTAINTY_GROUP_DEFAULT_COLORS = {
...
export function createDefaultOverallUncertaintyConfig() {
    const groups = [
        {
        ...
        },
        {
            ...
            name: "Не применимо",
            coef: 0,
            color: V2_UNCERTAINTY_GROUP_DEFAULT_COLORS[DEFAULT_GROUP_IDS.notApplicable],
        },
    ];
    const severityLevels = INITIATIVE_TIMELINE_VALUES.map((timelineLabel, i) => ({
        id: `sev_${i + 1}`,
        timelineLabel,
...
export function clampUncertaintyAdjustmentPct(pct, settings) {
...
export function resolveUncertaintyRiskCountCoef(count, ranges) {
 ...
 */
export function calculateOverallUncertaintyPreview(config, preview) {
    const formulaLines = [];
    if (!preview.enabled) {
        return {
            applicable: false,
...
export function resizeUncertaintyMatrix(config) {
...
export function isOverallUncertaintyConfigLogicRule(rule) {
    if (rule.id === V2_OVERALL_UNCERTAINTY_CONFIG_RULE_ID)
        return true;
    const payload = rule.payload;
    return (isPlainRecord(payload) &&
        payload.role === V2_OVERALL_UNCERTAINTY_CONFIG_PAYLOAD_ROLE);
}
export function parseOverallUncertaintyConfigFromLogic(rules) {
    const defaults = createDefaultOverallUncertaintyConfig();
...
export function mergeOverallUncertaintyConfigIntoLogic(rules, config) {
