...
export function formatKanbanBoardKey(projectCode, boardSlug) {
...
export function formatKanbanTaskKey(projectCode, taskNumber) {
    const project = normalizeTrackerCode(projectCode);
    return `${project}-${taskNumber}`;
}
export function parseKanbanTaskKey(key) {
 ...
 */
export function parseKanbanBoardKey(key, projectCodes) {
    const normalized = normalizeTrackerCode(key);
    if (!normalized)
        return null;
