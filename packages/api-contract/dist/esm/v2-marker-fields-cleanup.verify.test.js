...
describe("cleanup Маркер-дубликатов sourceSystems", () => {
    const anketa = JSON.parse(readFileSync(ANKETA, "utf8"));
    const worksSnap = JSON.parse(readFileSync(WORKS, "utf8"));
    ...
    it("работы ПиРМ матчятся по оставшимся paramCode с префиксом Маркер: в paramName", () => {
        ...
        for (const work of pirmWorks) {
            ...
            expect(rules
                .filter((r) => KEPT.includes(r.paramCode))
                .every((r) => /^Маркер:/i.test(r.paramName))).toBe(true);
            const source = {};
            for (const rule of rules) {
                if (rule.paramCode)
                    source[rule.paramCode] = true;
            }
            expect(matchTypicalWorkTriggers({ mode: "simple", rules, triggerArchCount: null, triggerFormula: null }, source, { detailInfo: { sourceSystems: [source] } })).toBe(true);
            const off = { ...source };
            const first = rules[0]?.paramCode;
            if (first)
                off[first] = false;
            expect(matchTypicalWorkTriggers({ mode: "simple", rules, triggerArchCount: null, triggerFormula: null }, off, { detailInfo: { sourceSystems: [off] } })).toBe(false);
        }
    });
});
