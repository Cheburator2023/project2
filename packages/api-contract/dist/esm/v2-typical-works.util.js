...
export function readTypicalWorksUniversalFormTriggerContext(data, uiSchema) {
    return readTypicalWorksRootFormTriggerContext(data, "", uiSchema);
}
export function hasTypicalWorkPreviewFormContext(data, uiSchema) {
    if (!data)
        return false;
...
export function buildTypicalWorkTriggerLookupSource(source, formData, referencePath = "detailInfo.sourceTypicalTasks", uiSchema) {
    if (!formData)
        return source;
...
export function readFilledArchComponentListRows(value) {
    if (Array.isArray(value)) {
