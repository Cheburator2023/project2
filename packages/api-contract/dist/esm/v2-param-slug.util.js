...
export function slugParamCode(name) {
    return name
        .toLowerCase()
...
export function normalizeStoredValueCode(code, label) {
    const trimmed = code.trim();
    if (!trimmed)
        return trimmed;
    if (trimmed.length <= MAX_STORED_VALUE_CODE_LENGTH)
        return trimmed;
    ...
    if (slug.length <= MAX_STORED_VALUE_CODE_LENGTH)
        return slug;
    return slug.slice(0, MAX_STORED_VALUE_CODE_LENGTH);
}
export function normalizeStoredValueLabel(label) {
...
export function resolveCatalogTriggerStoredValue(label) {
    const trimmed = label.trim();
    if (!trimmed)
