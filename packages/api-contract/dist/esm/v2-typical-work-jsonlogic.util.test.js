...
import { compileCalculationLogicFromVersionConfig, compileStoredTypicalWorkResultLogic, compileTypicalWorkCalculationLogic, compileTypicalWorkTriggerRulesToJsonLogic, compileWorkFormulaTokensToJsonLogic, computeTypicalWorkFormulaTotal, evaluateTypicalWorkCalculation, evaluateTypicalWorkJsonLogicValue, evaluateTypicalWorkResultJsonLogic, needsCalculationLogicBackfill, previewTypicalWorkCalculation, } from "./v2-typical-work-jsonlogic.util";
import { defaultWorkFormula, defaultWorkRounding } from "./v2-typical-work.types";
import { defaultBaseNormTerm } from "./v2-work-terms-formula.util";
...
describe("v2-typical-work-jsonlogic.util", () => {
    ...
    it("computeTypicalWorkFormulaTotal prefers token formula over simplified terms", () => {
        ...
        const calculationLogic = compileStoredTypicalWorkResultLogic({ tokens: parsed.tokens, text: "(N + 5) × коэф(p1)" }, defaultWorkRounding());
        const brokenTerms = {
            version: 2,
    ...
    it("compileCalculationLogicFromVersionConfig uses formulaText when formula is terms v2", () => {
        ...
        expect(compiled?.version).toBe(1);
        const result = evaluateTypicalWorkResultJsonLogic(compiled, { norm: 20, paramCoefficients: { p1: 1 } }, "(N + 5) × коэф(p1)");
        expect(result.value).toBe(25);
    });
    it("computeTypicalWorkFormulaTotal ignores stale norm-only calculationLogic", () => {
        const staleLogic = compileStoredTypicalWorkResultLogic(defaultWorkFormula(), defaultWorkRounding());
        const brokenTerms = {
            version: 2,
            terms: [defaultBaseNormTerm()],
            text: "H",
