...
export function triggerParamTokenToRule(token) {
    return {
...
export function describeTriggerFormulaToken(token) {
    switch (token.kind) {
        case "param":
...
export function compileTriggerFormulaTokensToJsonLogic(tokens) {
    if (tokens.length === 0)
        return false;
    const values = [];
    const opStack = [];
...
function evaluateTriggerOperand(token, ctx) {
    ...
    if (token.kind === "arch_count") {
        ...
        return archCountTriggerMatches(formData, token.archComponentKind, token.steps);
    }
    return false;
}
/** Вычисляет формулу триггеров (И/ИЛИ, скобки). Пустая → false. */
export function evaluateTriggerFormula(tokens, ctx) {
    if (tokens.length === 0)
        return false;
    let pos = 0;
    const parseOr = () => {
...
export function isTriggerFormulaConfigured(formula) {
