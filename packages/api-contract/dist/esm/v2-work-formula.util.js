...
export function reconcileFormulaLaborParamTokens(tokens, laborParams) {
...
export function validateWorkFormulaTokens(tokens, options) {
    ...
    for (let idx = 0; idx < tokens.length; idx++) {
        ...
        if (token.kind === "arch_count_coeff") {
            const stepsErr = validateArchCountCoeffSteps(token.archComponentKind, token.steps);
            if (stepsErr)
                return stepsErr;
        }
        expectOperand = false;
 ...
 */
export function cleanupWorkFormulaTokensAfterOperandRemoval(tokens) {
    let next = [...tokens];
    let changed = true;
    while (changed) {
        changed = false;
 ...
 */
export function repairWorkFormulaTokenOperators(tokens) {
 ...
 */
export function removeIncompatibleLaborKindFormulaTokens(tokens, laborParams) {
