...
import { flattenTypicalWorkStreamTriggerFields, hasTypicalWorkStreamTriggerContext, isFilledTypicalWorkSourceRow, readFilledArchComponentListRows, readTypicalWorksStreamTriggerContext, } from "./v2-typical-works.util";
describe("v2-typical-works.util stream trigger context", () => {
    it("flattens nested group fields by leaf key", () => {
        expect(flattenTypicalWorkStreamTriggerFields({
            groupKirilla: {
    ...
    it("reads model-stream MVP from generalInfo when output is controlTypicalTasks", () => {
        ...
        const context = readTypicalWorksStreamTriggerContext(formData, "generalInfo.modelService.controlTypicalTasks");
        expect(context.field_o_HRj6VO).toBe(true);
    });
    ...
    it("reads filled arch object list rows from array or singleton object", () => {
        expect(readFilledArchComponentListRows([{ field_o_HRj6VO: true }])).toEqual([{ field_o_HRj6VO: true }]);
        expect(readFilledArchComponentListRows({ field_o_HRj6VO: true })).toEqual([{ field_o_HRj6VO: true }]);
