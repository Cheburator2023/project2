...
describe("v2-work-arch-count-coeff.util", () => {
    ...
    it("validateArchCountCoeffSteps accepts formulas and rejects duplicates", () => {
        expect(validateArchCountCoeffSteps("sourceSystem", [
    ...
    it("formatLaborArchCountStepLabel uses operators and formulas", () => {
        expect(formatLaborArchCountStepLabel({
            count: 5,
            coefficient: 1,
            operator: "<=",
        })).toBe("≤5 → 1");
    ...
    it("archCountTriggerMatches supports comparison operators", () => {
        ...
        expect(archCountTriggerMatches(formWithTwoModels, "model", encodeTriggerArchCountSteps(">", 2))).toBe(false);
        expect(archCountTriggerMatches(formWithTwoModels, "model", encodeTriggerArchCountSteps("<=", 2))).toBe(true);
    });
    it("trigger encode/decode ignores labor-only fields", () => {
        ...
        expect(decodeTriggerArchCountCondition([
            {
                count: 5,
                coefficient: -2,
                operator: ">",
